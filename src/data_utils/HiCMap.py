...
class HiCMap: 
    ...
    def to_(self,*args,**kwargs):
        # In-place
        self.prob_map = self.prob_map.to(*args,**kwargs)
        return self
    ...
    def diagonal_mean(self,offset = 0):
        # to index first off-diagonal
        idx = torch.arange(self.nbins-offset, device=self.device)
        return self.prob_map[idx,idx+offset].nanmean() 

    def normalize_(self,first_off_diagonal_mean=1):
        # to index first off-diagonal
        idx = torch.arange(self.nbins-1, device=self.device)
        # Normalization factor
        nf = self.diagonal_mean(1) / first_off_diagonal_mean
        self.prob_map/= nf
        return self
        
    def normalize(self,first_off_diagonal_mean=1):
        # normalize to nearest neighbors
        return self.clone().normalize_(first_off_diagonal_mean)

    def normalize_as_(self,other_map):
        assert isinstance(other_map,HiCMap), \
        f"Expected HiCMap instance. Received {type(other_map).__name__}"
        
        return self.normalize_(other_map.diagonal_mean(1))
        
    def normalize_as(self,other_map):
        return self.clone().normalize_as_(other_map)
    ...
    def compute_insulation(self, window_size = 7):
        ws = window_size
        n = self.nbins
        v = self.values

        
        assert isinstance(ws,int) and 0 < ws < n//2, \
        f'window_size must be an integer. Received {type(ws).__name__}.'
        assert 0 < ws, f'window_size must be positive-valued. Received {ws:,}.'
        assert 2*ws < n, (
            f'ws must be less than half the number of bins in the Hi-C map ({n:,}/2={n/2}). '
            f'Received {ws:,}.'
        )
    
        # Get the window index that we'll slide as we take averages
        i,j = torch.meshgrid(torch.arange(-ws,0), torch.arange(1,ws+1),indexing='ij')

        # Computes the scores. Use NaN's when the window would extend beyond the map.
        scores = [torch.nan] * ws
        for k in range(ws, n - ws):
            scores.append(v[i+k,j+k].mean().log2())
        scores+= [torch.nan] * ws

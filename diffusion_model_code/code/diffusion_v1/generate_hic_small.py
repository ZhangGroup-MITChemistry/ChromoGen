...
#SBATCH --job-name=generate_hic_small
##SBATCH --partition=debug-gpu
#SBATCH --gres=gpu:volta:1
#SBATCH -c 20
##SBATCH -t 0-3:00:00
##SBATCH -t 0-36:00:00
#SBATCH --output=./log_files/generate_hic_small.log
...
sys.path.insert(0,'/home/gridsan/gschuette/refining_scHiC/revamp_with_zhuohan/code/diffusion_v1/')
...
embeddings = pd.read_pickle(f'../../data/embeddings_hg38/chrom_{chrom}.tar.gz')
...
def save_file(region,cond_scale,rescaled_phi,milestone,chrom):
    return f'./sampling_small/sample_{region}_{int(cond_scale)}_{int(10*rescaled_phi)}_{milestone}_{chrom}.pkl'
...
for region in range(0,1000,50): 
    for cond_scale in [1.]: #[float(k) for k in range(1,11)]:
        for rescaled_phi in [.5]: #[k/10 for k in range(1,11)]: 
            if os.path.exists(save_file(region,cond_scale,rescaled_phi,milestone,chrom)):
                continue
            ...
            sample.to('cpu') 
            pickle.dump(sample,open(save_file(region,cond_scale,rescaled_phi,milestone,chrom),'wb'))

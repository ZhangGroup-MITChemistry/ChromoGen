...
#SBATCH --job-name=sample_origami_64_resnet_like
##SBATCH --partition=debug-gpu
#SBATCH --ntasks=1 # Per NODE
#SBATCH --gres=gpu:volta:1  # Per NODE
#SBATCH --cpus-per-task=20  # Per TASK
#SBATCH --output=./log_files/sample_origami_64_resnet_like.log
...
nbeads = 64
use_embedding_reinsertion = True  # The newer embedding handler 
origami_fold = True
unet_dim = 64
milestone = 120
model_filepath = f'../../data/models/diffusion_origami_64_resnet_like/model-{milestone}.pt'
save_dir = '../../data/samples/origami_64_resnet_like/'
embedding_dir = '../../data/embeddings_65'
#chrom = '1'
#region_idx = 330
regions = { # chrom:region_idxs
  '1':[144,200,265,330,395,460,525,590,730,795,860,1260,1325], # already have samples for 330,395, so they're omitted
  'X':[100,236,381,445,553,610,675,810,900,965,1060,1125,1200]
}
cond_scales = [.9]   #[.3,.35,.4,.45,.5]
rescaled_phis = [.9] #[.3,.35,.4,.45,.5]

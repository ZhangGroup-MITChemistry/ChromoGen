...
#SBATCH --array=0-1
#SBATCH --output=./log_files/sample_origami_64_no_embed_reduction_%a_3.log
...
''' # Regions with interesting features
...
'''
# Regions with essentially NO features
regions = {
    '1':[],
    'X':[1100,1243]
}
    ...
    for f in embeddings_files:
        ...
        for cond_scale in cond_scales:
            for rescaled_phi in rescaled_phis:
                ...
                sf = sf+f.replace('.pt','')
                sf+= f'_{cond_scale}_{rescaled_phi}.pt'

                # Determine how many samples must still be generated
                n_samples_to_do = n_samples_remaining(sf,nsamples)

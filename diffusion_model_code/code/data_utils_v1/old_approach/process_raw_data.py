...
def load_coords(filepath):
    ...
    coord_df = pd.read_csv(
        ...
        sep='\t',
        header=None,
        names=['Chromosome','Genomic Index','x','y','z']
    )
...
def sort_coord_df(coord_df):
    ...
    coord_df.sort_values(
        by=['Lineage','Chromosome','Genomic Index'],
        axis='index',
        inplace=True,
        ignore_index=True
    )
...
def process_coord_data(coord_df,organism,cell_type,cell_number,replicate_number):
    '''
    ...
    # Insert columns for the organism, cell type, cell number, and structural
    # replicate information for easy navigation of the final HDF5 file.
    for col,val in [('Replicate',replicate_number),('Cell',cell_number),('Cell Type',cell_type),('Organism',organism)]:
        coord_df.insert(
            loc=0,
            column = col,
            value  = val

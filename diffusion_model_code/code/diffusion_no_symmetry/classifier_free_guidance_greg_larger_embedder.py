...
class EmbeddingEmbedder(nn.Module): 
    ...
    def __init__(
        ...
        dim_mults=(1,2,1,.5)#(1,.5,.25,.125) # assumes dim=64
    ):
        super().__init__()
...
class Unet(nn.Module):
    def __init__(
        ...
        classes_dim = dim * 4
        self.classes_mlp = EmbeddingEmbedder(64,classes_dim)
        '''
        self.null_classes_emb = nn.Parameter(torch.randn(embedding_dimensions))#dim))
        classes_dim = dim * 4
        c,h,w = embedding_dimensions
...
# This comes from https://github.com/lucidrains/denoising-diffusion-pytorch/blob/main/denoising_diffusion_pytorch/denoising_diffusion_pytorch.py
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.compute_subregion = ComputeSubregion(nbeads=65,includes_self_interaction=False) #Hardcoded here to get this training
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    '''
                    must take subset of each distance map! 
                    '''
                    dist_maps = self.compute_subregion(dist_maps)
